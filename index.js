...
class ChatHistoryStorage {
  ...
  unpinConversation(conversationId) {
    if (!conversationId || !this.isConversationPinned(conversationId)) return false;
...
class ChatHistoryUI extends DocumentManager{
  constructor() {
    ...
    this.chatContainer = this.querySelector(
      ".flex-col.flex-1.transition-opacity.duration-500.relative.-mr-2.pr-2.overflow-y-auto"
    );
    // this.RAW_HISTORY_ITEM = this.cloneElement(this.#getRawHistoryItem());
    this.initialize();
  }
  ...
  addPinnedConversationToUI({ title, conversationId }) {
    ...
    conversationLink.textContent = title;
  ...
  handleConversationHover = ({ target }) => {
    ...
    this.addPinButtonToConversation(target, { conversationId, conversationTitle });
  };
  ...
  loadPinnedConversations() {
    ...
    Object.entries(pinnedConversations).forEach(([conversationId, title]) => {
      this.addPinnedConversationToUI({ title, conversationId });
    });
  }
